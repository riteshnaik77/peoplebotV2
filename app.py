...
def sanitize_resume_text(text):
    ...
    # - 91-98765-43210, 91 98765 43210
    # - 98765-43210, 98765 43210, 9876543210 (Indian mobile)
    
    # International format with country code: +[1-3 digits] followed by number
    phone_pattern1 = r'\+?\d{1,3}[-.\s]?\(?\d{1,4}\)?[-.\s]?\d{1,4}[-.\s]?\d{1,9}'
    
    # US/Standard format: (123) 456-7890 or 123-456-7890 or 123.456.7890 or 123 456 7890
    phone_pattern2 = r'\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}'
    ...
    # Standalone 10-digit numbers (but exclude years 1900-2099 and common non-phone contexts)
    # Only match if it looks like a phone number (starts with 6-9 for Indian, or 2-9 for US)
    phone_pattern4 = r'\b([6-9]\d{9}|[2-9]\d{9})\b'
